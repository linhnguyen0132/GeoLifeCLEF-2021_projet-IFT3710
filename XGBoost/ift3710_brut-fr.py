...
print('XGBoost brut-2022')

# modif clusters = True pour utiliser les clusters et enlever les coords longtitude et latitude 
clusters = True
print('Utilisation clusters = ' + str(clusters))
#import cupy as cp
...
if clusters :
    nb_clusters = 100
    print('Nb de clusters = ' + str(nb_clusters))
    print('Utilisation méthode clustering au lieu des coords longitude et latitude')
...
y_train = le.transform(df_reduced.loc[obs_id_train]["species_id"].values)
y_val   = le.transform(df_reduced.loc[obs_id_val]["species_id"].values)

n_val = len(obs_id_val)
print("Validation set size: {} ({:.1%} of train observations)".format(n_val, n_val / len(df_obs)))

x_train = df_reduced.loc[obs_id_train].drop(columns=["species_id", "subset"])
x_val = df_reduced.loc[obs_id_val].drop(columns=["species_id", "subset"])
